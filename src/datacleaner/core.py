...
def detect_delimiter(file_path: Union[str, Path], encoding: str = 'utf-8-sig') -> str:
    ...
    recuento heurístico de los separadores más comunes.
    """
    path = Path(file_path)
    try:
...
def generate_report(file_path: Union[str, Path]) -> str:
    ...
    Acepta `Path` o `str` y delega en `inspect_csv` (pasa `encoding` opcional).
    """
    path = Path(file_path)
...
def clean_file(
    ...
    with open(in_path, newline='', encoding='utf-8') as infile, \
        ...
        for row in reader:
            ...
            # Escribir la fila directamente (streaming)
